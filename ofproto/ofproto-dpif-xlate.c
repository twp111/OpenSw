...
static void pof_do_xlate_actions(const struct ofpact *, size_t ofpacts_len,
                             struct xlate_ctx *);
static void pof_do_xlate_actions__(const struct ofpact *, size_t ofpacts_len,
                                 struct xlate_ctx *, struct pof_flow *,
                                 struct pof_fp_flow_wildcards *, int act_num, uint32_t fwd_acts);
...
xlate_recursively(struct xlate_ctx *ctx, struct rule_dpif *rule, bool deepens)
{
    ...
    VLOG_INFO("+++++++++++zq xlate_recursively:  before pof_do_xlate_actions");
    struct pof_fp_flow_wildcards *wc = ctx->wc;// zq
    struct pof_flow *flow = &ctx->xin->flow;
    int action_num = 1;
    uint32_t fwd_acts = 0x00;
    pof_do_xlate_actions__(actions->ofpacts, actions->ofpacts_len, ctx, flow, wc, action_num, fwd_acts);
    VLOG_INFO("+++++++++++zq xlate_recursively:  after pof_do_xlate_actions");
...
//        VLOG_INFO("+++++++++++sqy xlate_table_action:  after rule_dpif_lookup_from_table");
        ...
        if (rule) {
            ...
            VLOG_INFO("+++++++++++zq xlate_table_action:  before xlate_recursively");
            xlate_recursively(ctx, rule, table_id <= old_table_id);
            VLOG_INFO("+++++++++++zq xlate_table_action:  after xlate_recursively");
        }

        ctx->table_id = old_table_id;
        VLOG_INFO("+++++++++++zq xlate_table_action:  old_table_id:%d", old_table_id);
        return;
    }
}
                    ...
                    ofp_port_t port, uint16_t max_len, bool may_packet_in)
{
    ...
    case OFPP_IN_PORT:
        ...
        break;
    case OFPP_TABLE:
        VLOG_INFO("++++++tsf xlate_output_action: OFPP_TABLE");
    ...
    case OFPP_LOCAL:
    default:
        VLOG_INFO("++++++tsf xlate_output_action: default");
...
static void
pof_do_xlate_actions__(const struct ofpact *ofpacts, size_t ofpacts_len,
                     struct xlate_ctx *ctx, struct pof_flow *flow,
                     struct pof_fp_flow_wildcards *wc, int act_num, uint32_t fwd_acts)
{
    const struct ofpact *a;
...
//            int action_num = 0;
                ...
                flow->field_id[act_num] = htons(pf.field_id);
                flow->len[act_num] = htons(pf.len);
                flow->offset[act_num] = htons(pf.offset);
                flow->flag[act_num] = OFPACT_SET_FIELD;
...
//            int action_num = 0;
                ...
                flow->field_id[act_num] = htons(modify_field->field_id);
                flow->len[act_num] = htons(modify_field->len_field / 8);  // tsf: bytes
                flow->offset[act_num] = htons(modify_field->offset / 8);
                flow->flag[act_num] = OFPACT_MODIFY_FIELD;
                 ...
                 **/
                flow->value[act_num][0] = modify_field->increment;
                flow->mask[act_num][0] = 0xff;

                /*VLOG_INFO("pof_do_xlate_actions, before pof_fwd_acts_insert, fwd_acts: %08x\n", fwd_acts);*/
                pof_fwd_acts_insert(flow->field_id[act_num], a->type, &fwd_acts);
...
//            int action_num = 0;
                ...
                flow->field_id[act_num] = htons(add_field->tag_id);
                flow->len[act_num] = htons(add_field->tag_len / 8);  // bytes
                flow->offset[act_num] = htons(add_field->tag_pos / 8);
                flow->flag[act_num] = OFPACT_ADD_FIELD;
                /*VLOG_INFO("++++++tsf pof_do_xlate_actions: flow->field_id=%x", flow->field_id[action_num]);*/

                if (flow->field_id[act_num] == 0xffff) {  // tsf: execute add_dynamic_field, i.e. selective INT
                    wc->masks.sel_int_action = true;
                }

                pof_fwd_acts_insert(flow->field_id[act_num], a->type, &fwd_acts);

                memcpy(flow->value[act_num], add_field->tag_value, POF_MAX_FIELD_LENGTH_IN_BYTE);
                memset(flow->mask[act_num], 0xff, POF_MAX_FIELD_LENGTH_IN_BYTE);
...
//            int action_num = 0;
                ...
                flow->offset[act_num] = htons(delete_field->tag_pos);
                flow->len[act_num] = delete_field->len_type;
                flow->flag[act_num] = OFPACT_DELETE_FIELD;
                /*VLOG_INFO("++++++tsf pof_do_xlate_actions delete_field, offset=%d, len=%d", delete_field->tag_pos,
                        delete_field->len_type);*/

                struct pof_match *pm;
                memset(flow->value[act_num], 0x00, sizeof(flow->value[act_num]));
                memset(flow->mask[act_num], 0x00, sizeof(flow->mask[act_num]));
                if (flow->len[act_num] == 0) { // POFVT_IMMEDIATE_NUM, will cut 32b to 16 b in commit_odp_actions()
                    flow->field_id[act_num] = 0;
                    memcpy(flow->value[act_num], &delete_field->tag_len.value, sizeof(delete_field->tag_len.value));
                    memset(flow->mask[act_num], 0xff, sizeof(delete_field->tag_len.value));
                    /*VLOG_INFO("++++++tsf pof_do_xlate_actions offset=%d, len=%d", delete_field->tag_pos, delete_field->tag_len.value);*/
                } else {  // // POFVT_FIELD
                    flow->field_id[act_num] = htons(delete_field->tag_len.field.field_id);
                    memcpy(flow->value[act_num], &delete_field->tag_len, sizeof(delete_field->tag_len));  // tsf: copy all union
                    memset(flow->mask[act_num], 0xff, sizeof(delete_field->tag_len));
...
//    const struct ofpact *a;
    ...
    pof_do_xlate_actions__(ofpacts, ofpacts_len, ctx, flow, wc, action_num, fwd_acts);
                 ...
                 struct xlate_ctx *ctx)
{
    ...
    OFPACT_FOR_EACH (a, ofpacts, ofpacts_len) {
        ...
        case OFPACT_OUTPUT:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_OUTPUT");
            xlate_output_action(ctx, ofpact_get_OUTPUT(a)->port,
                                ofpact_get_OUTPUT(a)->max_len, true);
            break;

        case OFPACT_GROUP:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_GROUP");
            if (xlate_group_action(ctx, ofpact_get_GROUP(a)->group_id)) {
                /* Group could not be found. */
        ...
        case OFPACT_CONTROLLER:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_CONTROLLER");
        ...
        case OFPACT_ENQUEUE:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_ENQUEUE");
        ...
        case OFPACT_SET_VLAN_VID:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_SET_VLAN_VID");
            wc->masks.vlan_tci |= htons(VLAN_VID_MASK | VLAN_CFI);
        ...
        case OFPACT_SET_VLAN_PCP:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_SET_VLAN_PCP");
        ...
        case OFPACT_STRIP_VLAN:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_STRIP_VLAN");
            memset(&wc->masks.vlan_tci, 0xff, sizeof wc->masks.vlan_tci);
            flow->vlan_tci = htons(0);
            break;

        case OFPACT_PUSH_VLAN:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_PUSH_VLAN");
            /* XXX 802.1AD(QinQ) */
            memset(&wc->masks.vlan_tci, 0xff, sizeof wc->masks.vlan_tci);
            flow->vlan_tci = htons(VLAN_CFI);
            break;
        ...
        case OFPACT_SET_ETH_SRC:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_SET_ETH_SRC");
            WC_MASK_FIELD(wc, dl_src);
            flow->dl_src = ofpact_get_SET_ETH_SRC(a)->mac;
            break;

        case OFPACT_SET_ETH_DST:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_SET_ETH_DST");
            WC_MASK_FIELD(wc, dl_dst);
            flow->dl_dst = ofpact_get_SET_ETH_DST(a)->mac;
            break;

        case OFPACT_SET_IPV4_SRC:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_SET_IPV4_SRC");
            if (flow->dl_type == htons(ETH_TYPE_IP)) {
                memset(&wc->masks.nw_src, 0xff, sizeof wc->masks.nw_src);
        ...
        case OFPACT_SET_IPV4_DST:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_SET_IPV4_DST");
            if (flow->dl_type == htons(ETH_TYPE_IP)) {
                memset(&wc->masks.nw_dst, 0xff, sizeof wc->masks.nw_dst);
        ...
        case OFPACT_SET_IP_DSCP:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_SET_IP_DSCP");
            if (is_ip_any(flow)) {
                wc->masks.nw_tos |= IP_DSCP_MASK;
                flow->nw_tos &= ~IP_DSCP_MASK;
        ...
        case OFPACT_SET_IP_ECN:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_SET_IP_ECN");
            if (is_ip_any(flow)) {
        ...
        case OFPACT_SET_IP_TTL:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_SET_IP_TTL");
            if (is_ip_any(flow)) {
        ...
        case OFPACT_SET_L4_SRC_PORT:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_SET_L4_SRC_PORT");
        ...
        case OFPACT_SET_L4_DST_PORT:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_SET_L4_DST_PORT");
            if (is_ip_any(flow) && !(flow->nw_frag & FLOW_NW_FRAG_LATER)) {
                memset(&wc->masks.nw_proto, 0xff, sizeof wc->masks.nw_proto);
        ...
        case OFPACT_RESUBMIT:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_RESUBMIT");
            /* Freezing complicates resubmit.  Some action in the flow
             * entry found by resubmit might trigger freezing.  If that
             * happens, then we do not want to execute the resubmit again after
             * during thawing, so we want to skip back to the head of the loop
        ...
        case OFPACT_SET_TUNNEL:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_SET_TUNNEL");
            flow->tunnel.tun_id = htonll(ofpact_get_SET_TUNNEL(a)->tun_id);
            break;

        case OFPACT_SET_QUEUE:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_SET_QUEUE");
            memset(&wc->masks.skb_priority, 0xff,
                   sizeof wc->masks.skb_priority);
        ...
        case OFPACT_POP_QUEUE:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_POP_QUEUE");
            memset(&wc->masks.skb_priority, 0xff,
                   sizeof wc->masks.skb_priority);
        ...
        case OFPACT_REG_MOVE:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_REG_MOVE");
            mf_subfield_copy(&ofpact_get_REG_MOVE(a)->src,
                             &ofpact_get_REG_MOVE(a)->dst, flow, wc);
            break;

        case OFPACT_SET_FIELD:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_SET_FIELD");
            set_field = ofpact_get_SET_FIELD(a);
            mf = set_field->field;
        ...
        case OFPACT_STACK_PUSH:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_STACK_PUSH");
            nxm_execute_stack_push(ofpact_get_STACK_PUSH(a), flow, wc,
                                   &ctx->stack);
            break;

        case OFPACT_STACK_POP:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_STACK_POP");
            nxm_execute_stack_pop(ofpact_get_STACK_POP(a), flow, wc,
                                  &ctx->stack);
            break;

        case OFPACT_PUSH_MPLS:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_PUSH_MPLS");
            compose_mpls_push_action(ctx, ofpact_get_PUSH_MPLS(a));
            break;

        case OFPACT_POP_MPLS:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_POP_MPLS");
            compose_mpls_pop_action(ctx, ofpact_get_POP_MPLS(a)->ethertype);
            break;

        case OFPACT_SET_MPLS_LABEL:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_SET_MPLS_LABEL");
            compose_set_mpls_label_action(
                ctx, ofpact_get_SET_MPLS_LABEL(a)->label);
            break;

        case OFPACT_SET_MPLS_TC:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_SET_MPLS_TC");
            compose_set_mpls_tc_action(ctx, ofpact_get_SET_MPLS_TC(a)->tc);
            break;

        case OFPACT_SET_MPLS_TTL:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_SET_MPLS_TTL");
            compose_set_mpls_ttl_action(ctx, ofpact_get_SET_MPLS_TTL(a)->ttl);
            break;

        case OFPACT_DEC_MPLS_TTL:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_DEC_MPLS_TTL");
        ...
        case OFPACT_DEC_TTL:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_DEC_TTL");
            wc->masks.nw_ttl = 0xff;
        ...
        case OFPACT_NOTE:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_NOTE");
            /* Nothing to do. */
            break;

        case OFPACT_MULTIPATH:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_MULTIPATH");
            multipath_execute(ofpact_get_MULTIPATH(a), flow, wc);
            break;

        case OFPACT_BUNDLE:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_BUNDLE");
            xlate_bundle_action(ctx, ofpact_get_BUNDLE(a));
            break;

        case OFPACT_OUTPUT_REG:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_OUTPUT_REG");
            xlate_output_reg_action(ctx, ofpact_get_OUTPUT_REG(a));
            break;

        case OFPACT_OUTPUT_TRUNC:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_OUTPUT_TRUNC");
            xlate_output_trunc_action(ctx, ofpact_get_OUTPUT_TRUNC(a)->port,
                                ofpact_get_OUTPUT_TRUNC(a)->max_len);
            break;

        case OFPACT_LEARN:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_LEARN");
            xlate_learn_action(ctx, ofpact_get_LEARN(a));
            break;

        case OFPACT_CONJUNCTION: {
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_CONJUNCTION");
        ...
        case OFPACT_EXIT:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_EXIT");
            ctx->exit = true;
            break;

        case OFPACT_UNROLL_XLATE: {
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_UNROLL_XLATE");
        ...
        case OFPACT_FIN_TIMEOUT:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_FIN_TIMEOUT");
            memset(&wc->masks.nw_proto, 0xff, sizeof wc->masks.nw_proto);
            xlate_fin_timeout(ctx, ofpact_get_FIN_TIMEOUT(a));
            break;

        case OFPACT_CLEAR_ACTIONS:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_CLEAR_ACTIONS");
            ofpbuf_clear(&ctx->action_set);
            ctx->xin->flow.actset_output = OFPP_UNSET;
        ...
        case OFPACT_WRITE_ACTIONS:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_WRITE_ACTIONS");
            xlate_write_actions(ctx, ofpact_get_WRITE_ACTIONS(a));
            break;

        case OFPACT_WRITE_METADATA:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_WRITE_METADATA");
        ...
        case OFPACT_METER:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_METER");
            /* Not implemented yet. */
            break;

        case OFPACT_GOTO_TABLE: {
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_GOTO_TABLE");
            struct ofpact_goto_table *ogt = ofpact_get_GOTO_TABLE(a);
        ...
        case OFPACT_SAMPLE:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_SAMPLE");
            xlate_sample_action(ctx, ofpact_get_SAMPLE(a));
            break;

        case OFPACT_CT:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_CT");
            compose_conntrack_action(ctx, ofpact_get_CT(a));
            break;

        case OFPACT_NAT:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_NAT");
            /* This will be processed by compose_conntrack_action(). */
            ctx->ct_nat_action = ofpact_get_NAT(a);
            break;

        case OFPACT_DEBUG_RECIRC:
            VLOG_INFO("+++++++zq do_xlate_actions OFPACT_DEBUG_RECIRC");
            ctx_trigger_freeze(ctx);
            a = ofpact_next(a);
            break;
        }
         ...
         * execution. */
        if (!ctx->error && ctx->exit && ctx_first_frozen_action(ctx)) {
            VLOG_INFO("+++++++zq do_xlate_actions Check if need to store this and the remaining actions");
            freeze_unroll_actions(a, ofpact_end(ofpacts, ofpacts_len), ctx);
            break;
        }
    }
